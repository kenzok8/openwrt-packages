...
new_clashdtun_core_version=`wget -qO- "https://hub.fastgit.xyz/Dreamacro/clash/releases/tag/premium"| grep "/download/premium/"| head -n 1| awk -F " " '{print $2}'| awk -F "-" '{print $4}'| sed "s/.gz\"//g"`
...
new_clashtun_core_version=`wget -qO- "https://hub.fastgit.xyz/comzyh/clash/tags"| grep "/comzyh/clash/releases/"| head -n 1| awk -F "/tag/" '{print $2}'| sed 's/\">//g'`
...
new_clashr_core_version=`wget -qO- "https://hub.fastgit.xyz/Dreamacro/clash/tags"| grep "/Dreamacro/clash/releases/"| head -n 1| awk -F "/tag/" '{print $2}'| sed 's/\">//g'`
...
update(){
	   ...
	   if [ $CORETYPE -eq 1 ];then
		wget --no-check-certificate  https://hub.fastgit.xyz/Dreamacro/clash/releases/download/"$CLASHVER"/clash-"$MODELTYPE"-"$CLASHVER".gz -O 2>&1 >1 /tmp/clash.gz
	   elif [ $CORETYPE -eq 3 ];then 
		wget --no-check-certificate  https://hub.fastgit.xyz/comzyh/clash/releases/download/"$CLASHTUN"/clash-"$MODELTYPE"-"$CLASHTUN".gz -O 2>&1 >1 /tmp/clash.gz
	   elif [ $CORETYPE -eq 4 ];then 
		wget --no-check-certificate  https://hub.fastgit.xyz/Dreamacro/clash/releases/download/premium/clash-"$MODELTYPE"-"$CLASHDTUNC".gz -O 2>&1 >1 /tmp/clash.gz
	   fi
